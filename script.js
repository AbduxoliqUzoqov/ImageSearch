...
const errorText = lightbox.querySelector(".error");
...
function tarjima(tx){
  const apitext = `https://api.mymemory.translated.net/get?q=${tx}&langpair=uz|en`;
  fetch(apitext).then(res => res.json()).then(data => {
    const searchTerm = data.responseData.translatedText;
...
const loadSearchImages = (e) => {
    ...
    if (e.key === "Enter") {
        ...
        //searchTerm = tarjima(searchTerm1);
